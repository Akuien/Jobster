...
router.post('/login', function(req, res ) {
    if(req.body.company_email && req.body.password){
        ...
        const Company= Company.findOne({email}, function(err, company){
            if(err){
            ...
            bcrypt.compare(req.body.password, company[0].password, function(err, feedback){
                if(err){
                    return res.status(401).json({
                        message: 'Please provide valid email and/or password'
                    });
